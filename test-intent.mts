...
onAnyEvent((e) => console.log(`[EVENT] ${e.type}:`, JSON.stringify(e.payload).slice(0, 200)));

console.log('Testing intent emission...');
const res = await call(subagent,{
  task: 'Research the top 3 AI agent frameworks in 2024 and list their key features.',
  agentType: 'researcher',
  model: 'mistral-small-latest',
  maxSteps: 10,
});
console.log('\n--- RESULT ---');
console.log('Output length:', res.result?.length);
console.log('Steps:', res.steps);
console.log('Tool calls:', res.toolCalls);
console.log('Tool results:', res.toolResults?.length);