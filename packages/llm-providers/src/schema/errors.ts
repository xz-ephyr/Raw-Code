...
export class HttpRateLimitDetails extends Schema.Class<HttpRateLimitDetails>("LLM.HttpRateLimitDetails")({
  ...
  limit: Schema.optional(Schema.Record({ key: Schema.String, value: Schema.String })),
  remaining: Schema.optional(Schema.Record({ key: Schema.String, value: Schema.String })),
  reset: Schema.optional(Schema.Record({ key: Schema.String, value: Schema.String })),
}) {}
...
export class NoRouteReason extends Schema.Class<NoRouteReason>("LLM.Error.NoRoute")({
  ...
  get retryable() { return false }
  get message() { return `No LLM route for ${this.provider}/${this.model} using ${this.route}` }
}
...
export class AuthenticationReason extends Schema.Class<AuthenticationReason>("LLM.Error.Authentication")({
  ...
  message: Schema.String,
  kind: Schema.Literal("missing", "invalid", "expired", "insufficient-permissions", "unknown"),
  providerMetadata: Schema.optional(ProviderMetadata),
  http: Schema.optional(HttpContext),
}) {
  get retryable() { return false }
...
export class LLMError extends Schema.TaggedClass<LLMError>()("LLM.Error", {
  module: Schema.String,
  method: Schema.String,
  reason: LLMErrorReason,
}) {
  get cause() { return this.reason }

  get retryable() { return (this.reason as any).retryable }
  get retryAfterMs() { return "retryAfterMs" in this.reason ? (this.reason as any).retryAfterMs : undefined }
  get message() { return `${this.module}.${this.method}: ${(this.reason as any).message}` }
}

export class ToolFailure extends Schema.TaggedClass<ToolFailure>()("LLM.ToolFailure", {
  message: Schema.String,
