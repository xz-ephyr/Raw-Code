...
describe("face parameters", () => {
  ...
  it("computeLandmarks produces valid head and neck", () => {
    const rng = createRNG(12345)
    const p = generateFaceParams(rng)
    const lm = computeLandmarks(p)
    expect(lm.head.rx).toBeGreaterThan(0)
    expect(lm.head.ry).toBeGreaterThan(0)
    expect(lm.neck.rx).toBeGreaterThan(0)
    expect(lm.neck.botY).toBeGreaterThan(lm.neck.topY)
    expect(lm.leftEye.w).toBeGreaterThan(0)
    expect(lm.rightEye.w).toBeGreaterThan(0)
    expect(lm.nose.tip.x).toBeGreaterThan(0)
    expect(lm.mouth.halfW).toBeGreaterThan(0)
  })

  it("palette generation produces valid greyscale", () => {
    const rng = createRNG(67890)
    const palette = generatePalette(rng)
    expect(palette.skin.r).toBe(palette.skin.g)
    expect(palette.skin.g).toBe(palette.skin.b)
    expect(palette.hair.r).toBe(palette.hair.g)
