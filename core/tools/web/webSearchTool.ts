...
export const imageSearchTool = {
  ...
  ...tool({
    ...
    inputSchema: zodSchema(z.object({
      ...
      maxResults: z.number().optional().default(10).describe('Number of image results (1–10). Hard cap: 10 per search.'),
      safeSearch: z.boolean().optional().default(true).describe('Filter out explicit content.'),
    })),
    execute: async ({ query, maxResults, safeSearch }) => {
      try {
        const result = await WebSearchService.imageSearch({ query, maxResults: Math.min(maxResults ?? 10, 10), safeSearch });
        return {
          results: result.results.map(r => ({
...
export const newsSearchTool = {
  ...
  ...tool({
    ...
    inputSchema: zodSchema(z.object({
      ...
      maxResults: z.number().optional().default(10).describe('Number of news results (1–10). Hard cap: 10 per search.'),
    ...
    execute: async ({ query, maxResults, freshness }) => {
      try {
        const result = await WebSearchService.newsSearch({ query, maxResults: Math.min(maxResults ?? 10, 10), freshness });
        return {
          results: result.results.map(r => ({
            title: r.title, url: r.url, snippet: r.snippet,
