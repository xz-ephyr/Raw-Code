...
const research = mat.definitionsMap.get('research');
let c=0; const ctx=()=>({sessionID:'s'+c,agentID:'t',assistantMessageID:'m'+c,toolCallID:'c'+c,resolveModel:(n)=>n});
const call=async(tool,input)=>{c++;const r=await tool.settle({id:'k'+c,name:'research',input},ctx());if(r.type==='error')throw new Error(r.message);return r.value;};

console.log('Testing research tool directly...');
const res = await call(research,{query:'current state of AI agents in software development', depth:'deep', maxSources:5});
console.log('Result:', JSON.stringify(res).slice(0, 1000));