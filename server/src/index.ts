...
// --- API Proxy (bypass CORS for provider APIs) ---
...
app.post('/get_project_memory', async (req, res) => {
  const { projectId } = req.body;
  if (!projectId) return res.status(400).json({ error: 'projectId required' });
  const result = await query(
...
app.post('/set_project_memory', async (req, res) => {
...
app.post('/delete_project_memory', async (req, res) => {
...
app.post('/clear_project_memory', async (req, res) => {
  const { projectId } = req.body;
  if (!projectId) return res.status(400).json({ error: 'projectId required' });
...
app.use('/proxy/', async (req, res) => {
  // req.path is like /https:/generativelanguage.googleapis.com/v1beta/openai/chat/completions
  const raw = req.path.replace(/^\//, ''); // strip leading slash
  // Restore double slash after scheme: https:/host -> https://host
  const actualUrl = raw.replace(/^(https?:)\/([^/])/, '$1//$2');
